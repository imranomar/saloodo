...
```bash
curl -X GET -G "/api/product" 
```

```javascript
const url = new URL("/api/product");
...
```bash
curl -X GET -G "/api/product/1" 
```

```javascript
const url = new URL("/api/product/1");
